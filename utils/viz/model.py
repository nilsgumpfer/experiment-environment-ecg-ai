  ...
  """
    ...
    for i, layer in enumerate(layers):
        ...
        if label.startswith('Conv'):
            ...
            color = 'coral1'
        elif label in ['ELU', 'ReLU', 'LeakyReLU', 'Softmax']:
            color = 'orange'
        elif label.startswith('MaxPooling'):
            color = 'deepskyblue'
        elif label == 'Flatten':
            color = 'yellow'
        elif label.startswith('GlobalAveragePooling'):
            color = 'yellow'
        elif label == 'Dense':
            color = 'powderblue'
        else:
        ...
        if not expand_nested or not isinstance(layer, network.Network):
            node = pydot.Node(layer_id, label=label, color=color, style='filled', fontname='Courier')
            dot.add_node(node)
