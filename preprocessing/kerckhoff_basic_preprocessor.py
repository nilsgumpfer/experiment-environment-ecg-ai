...
from utils.data.data import scale_ecgs, derive_ecg_variants_multi, crop_ecgs, \
    ...
    combine_ecgs_and_clinical_parameters, save_dataset, load_metadata, load_ecgs_from_redcap_snapshots, \
    load_clinical_parameters_from_redcap_snapshots


class KerckhoffBasicPreprocessor(AbstractPreprocessor):
    ...
    def perform_preprocessing(self):
        # 1. Load ECGs
        original_ecgs = load_ecgs_from_redcap_snapshots(self.params['snapshot_id'], self.params['leads_to_use'], self.params['record_ids_excluded'])
        ...
        # 3. Crop ECGs
        cropped_ecgs = crop_ecgs(scaleded_ecgs, self.params['crop_id'])  # TODO: Implement crop marker file creation
        logging.info('Cropped ECGs')

        # 4. Further ECG derivation
        derived_ecgs = derive_ecg_variants_multi(cropped_ecgs, self.params['ecg_variants'])
        logging.info('Derived further ECG variants')

        # 5. Load clinical parameters
        clinical_parameters = load_clinical_parameters_from_redcap_snapshots(self.params['snapshot_id'], self.params['clinical_parameters_inputs'], self.params['clinical_parameters_outputs'], self.params['record_ids_excluded'])
