...
class AbstractRunner(ABC):
    ...
    def __init__(self, exp_id=None, default_yes=False):
        ...
        parser.add_argument('-e', '--exp', type=str, help='Experiment identifier', required=True)
        parser.add_argument('--yes', action='store_true', help='Skip questions, always answer YES', required=False)
        parser.add_argument('--suffix', type=str, help='Subexperiment suffix (only for explanation)', required=False)
        parser.add_argument('--epoch', type=int, help='Epoch (only for explanation)', required=False)
